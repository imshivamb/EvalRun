...
For v2.1, validation is not conditional on the reflection outcome. Every
replanning draft produces a structured summary and invokes MCP. If reflection
approves and MCP passes, that draft is returned. If either system finds a
problem, the planner revises using both feedback sources and MCP records a
second validation of the final draft.
...
Both sides run in `closed_world_evaluation` planning mode. This benchmark
deliberately contains its own day numbers, disruption facts, and constraints;
the planner must produce a provisional itinerary rather than defer for calendar
dates or live availability. The same mode is used on both sides, so MCP remains
the sole intended intervention.
